$(function(){
    ...
    if ($('#showcart').length > 0 ){
        ...
        $('.editcount').css('cursor','pointer').dblclick(function(){
            ...
            var inp = $('<input id="'+id+'inp" type="text" style="width:15px; border:1px solid #000;" value="'+val+'" />').blur(function(){
    ...
    function providers(addressid,f){
        $('#copyright').text(addressid+' Editus');
...
//        }
        if ($('#so1_addresses_sel').val()=='new'){
            ...
            providers($('#so1_addresses_sel').val(),false);
        }
        ...
        $('#saveandselnewaddress').click(function(){
            $.post(main_file_name+'?do=shoporderstep1&a=1',{'do':'savenewaddress',
                   ...
                   $('#newaddress').fadeOut();
                   providers($('#so1_addresses_sel').val(),true);
            });
        });
