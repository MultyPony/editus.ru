...
function listusers(&$e,&$t) {
    ...
    $db->query("SELECT Users.userId, Users.userFirstName, Users.userLastName,
                ...
                FROM Users, Groups
                WHERE Groups.groupId = Users.userGroupId ORDER BY Users.userRegistrationDate DESC");
...
function usersmailer(){
    ...
    if (isset($_POST['subj']) && isset($_POST['umails']) && isset($_POST['mess'])){
        ...
        $db->query("INSERT INTO UsersMailer
                    SET usersEmail = '".$sm."',
                        remainUsersEmail = '".$sm."',
                        subj = '".$db->mres($_POST['subj'])."',
                        mess = '".$db->mres($_POST['mess'])."' ");
    }
...
function supportadmin(&$e) {
    ...
    $data['sc_cat']=$_POST['sc_cat'];
     if (isset($_POST['sc_message'])){
       ...
       $t = $db->fetch_array();
       $e->mail($t[0], 'Новое сообщение', 'Вы получили ответ на Ваше сообщение в службу техподдержки. Для прочтения перейдите по ссылке - //<?php echo $_SERVER['SERVER_NAME'];?>/editus.php?do=viewsupportmess&id='.intval($_POST['replyId']));
    }
    $datamess = array();
