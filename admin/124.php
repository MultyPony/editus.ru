...
//        while(true){
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, 'http://missmo.ru/20102.php');
        curl_setopt($curl, CURLOPT_HEADER, 1);
        curl_setopt($curl, CURLOPT_POST, 1);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($curl, CURLOPT_POSTFIELDS, array( 'gl'=>urlencode(10) ));
        curl_setopt($curl, CURLOPT_USERAGENT, 'Opera/8.54 (Windows NT 5.1; U; en)');
        $res = curl_exec($curl);
        if(!$res){
